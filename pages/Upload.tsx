...
import { categorizeTransaction, analyzeReceipt, mapCsvHeaders, parseBankStatement, analyzePortfolioPDF } from '../services/geminiService';

import { extractTextFromPdf } from '../services/pdfService';
import { TransactionCategory, PortfolioAnalysis } from '../types';
import { UploadCloud, Check, AlertCircle, FileText, Camera, Loader2, File, PieChart, TrendingUp, Shield, Save } from 'lucide-react';
...
export default function Upload() {
    ...
    const [tab, setTab] = useState<'csv' | 'receipt' | 'pdf' | 'portfolio'>('csv');
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 pb-20">
            <div className="flex gap-4 border-b border-gray-200 overflow-x-auto">
                {['csv', 'pdf', 'portfolio', 'receipt'].map((t) => (
                    <button
                        key={t}
                    ...
                    >
                        {t === 'pdf' ? 'Bank Stmt (PDF)' : t === 'portfolio' ? 'Portfolio (PDF)' : t === 'csv' ? 'Bulk CSV' : 'Scan Receipt'}
                    </button>
                ))}
            </div>
